...
func load_data(path: String):
	if FileAccess.file_exists(path):
		...
		for key in data.global_data.player_list.keys():
			...
			global_data.player_list[key].flag = current_player.flag
	else:
...
func _createPlayerList():
	...
	if len(global_data.player_list.keys()) > 0:
		...
		for key in global_data.player_list.keys():
			...
			grid.get_children()[i].get_children()[0].get_children()[0].get_children()[0].get_children()[0].set_texture(load(global_data.player_list[key].flag))
			i += 1
...
func _on_button_pressed():
	...
	if len(name) > 3:
		if result:
			...
			global_data.player_list[str(name)].flag = country_flag.texture.resource_path
